...
nasm -felf32 -o boot.o boot2.S
gcc -c kernel2.c -o kernel.o -nostdlib
gcc link2.ld boot.o kernel.o -o kernel.elf -nostdlib
nasm model2.asm -o model.o
objdump -D kernel.elf > out.txt
cat out.txt | grep kernel_main
dd if=kernel.elf of=hello2.c32
dd if=model.o of=hello2.c32 count=1 conv=notrunc
cp *.com CD_root/isolinux/
cp *.c32 CD_root/isolinux/
